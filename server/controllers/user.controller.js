...
const getUser = catchAsync(async (req, res) => {
    const { id } = req.params;
    const data = await attachQueries(User.findById(id), req.query).orFail(new AppError('User not found.', 404));
    res.status(200).json({
        status: 'success',
        data,
    });
...
const updateSelf = catchAsync(async (req, res, next) => {
    ...
    const data = await User.findByIdAndUpdate(id, {
        ...
        userBanner,
        userPic,
    }, { new: true, runValidators: true }).orFail(new AppError('User profile deleted.', 404));
    if (userBanner) {
...
const addRemoveFriend = catchAsync(async (req, res, next) => {
    ...
    const foundFriend = await User.findById(friendID).orFail(new AppError('Friend not found.', 404));
    ...
    const data = await User.findByIdAndUpdate(userID, { friends }, { new: true, runValidators: true })
